...
(function (preact, signals, hooks, dayjs, CryptoJS) {
  ...
  function InsightsTab() {
    ...
    const [youtubeService] = hooks.useState(() => new YouTubeService());
    ...
    const isVideoPage = () => {
      return youtubeService.isVideoPage();
    };
    ...
    const loadChannelInfo = async () => {
      ...
      if (isVideoPage()) {
        const vId = youtubeService.getVideoIdFromURL();
        if (vId !== videoId) {
          setVideoId(vId);
          setChannelId(null);
      ...
      const id = await youtubeService.getChannelId();
      const handle = youtubeService.getChannelHandle();
      if (id !== channelId) {
        setChannelId(id);
    ...
    const loadTabCounts = async () => {
      ...
      await Promise.all(
        tabs.map(async (tab) => {
          const count = await youtubeService.fetchTabCount(
            tab.type,
    ...
    hooks.useEffect(() => {
      ...
      const checkUrlChange = () => {
        const currentHandle = youtubeService.getChannelHandleFromURL();
        const currentVideoId = youtubeService.getVideoIdFromURL();
    ...
    return u("div", { className: "space-y-4", children: [
      videoInfo && u(preact.Fragment, { children: [
        youtubeService.isVideoPage() && window.location.pathname.startsWith("/shorts/") ? u("div", { className: "grid grid-cols-2 gap-2", children: ["oardefault", "oar1", "oar2", "oar3"].map((quality) => u(
          "a",
          {
...
new Set()
    ...
    const loadChannelInfo = async () => {
      ...
      if (isWatchPage()) {
        ...
        if (videoId !== currentVideoId) {
          ...
          setCurrentVideoTitle(getVideoTitle());
    ...
    hooks.useEffect(() => {
      ...
      const checkUrlChange = () => {
        const currentHandle = youtubeService.getChannelHandleFromURL();
        if (currentHandle !== youtubeService.getChannelHandle()) {
          youtubeService.clearCache();
          loadChannelInfo();
        }
